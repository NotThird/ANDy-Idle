...
export default class PlayerManager {
  constructor(player, scene, enemies) {
    ...
    this.player.attackCollider = this.scene.physics.add.sprite(this.player.x, this.player.y, null);
    this.player.attackCollider.setSize(this.player.width * 2, this.player.height * 2);
  ...
  attack() {
    if (Phaser.Input.Keyboard.JustDown(this.keyQ) && !this.isAttacking && !this.attackCooldown) {
  ...
  update() {
    ...
    const isMoving = updateMovement(this.player, this.keys, this.isAttacking, this.isRunning);
...
updateHealthBar() {
  ...
  const remainingHealth = (this.player.health / this.player.maxHealth) * healthBarWidth; // Access player's health directly
